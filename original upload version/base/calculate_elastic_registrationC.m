...
if ~exist('cutoff','var');cutoff=0.15;end
    ...
    for kk=find(checkS>cutoff)'
        ...
        immvS2=zeros(size(immvS));
        immvS2(cc2:end-cc,cc2:end-cc,:)=immvS(cc2:end-cc,cc2:end-cc,:);
        [X,Y,imoutS,RS]=reg_ims_ELS(immvS,imrfS,2,1);a=immvS2(immvS2>0 & imrfS>0);
        b=imrfS(immvS2>0 & imrfS>0);R0=corr2(a,b);
        %[XS,YS,imoutS,RS]=reg_ims_ELS(immvS,imrfS,2);a=immvS(immvS>0 & imrfS>0);b=imrfS(immvS>0 & imrfS>0);R0=corr2(a,b);
        RR=[R0 RS];
